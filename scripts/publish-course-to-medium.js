...
function getBody(course_slug, slug) {
  ...
  body = body.replace(
    /\]\(\/static\/courses/g,
    '](https://raw.githubusercontent.com/venkateshreddypala/portfolio/master/public/static/courses',
  );

  // Replace static links
  body = body.replace(
    new RegExp(`]\\(/courses/${course_slug}`, 'g'),
    `](https://vrpala.com/courses/${course_slug}`,
  );
...
_This article is part of my open-source [Machine Learning Course](https://github.com/venkateshreddypala/machine-learning) available on Github._`;
...
(async function main() {
  ...
  for (const [, { name, slug }] of articles.entries()) {
    ...
    const canonicalUrl = `https://github.com/venkateshreddypala/${course_slug}#${slug}`;
