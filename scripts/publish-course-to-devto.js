...
//@To-do Update this to my context
function getBanner(section, course_slug, slug) {
  return `https://raw.githubusercontent.com//portfolio/master/public/static/courses/${course_slug}/${section}/${slug}/banner.png`;
}
...
function getBody(course_slug, slug) {
  ...
  //@To-do Replace direct static images with github links
  let body = frontmatter.content;

  body = body.replace(
    /\]\(\/static\/courses/g,
    '](https://raw.githubusercontent.com//portfolio/master/public/static/courses',
  );

  // @To-do Replace static links
  body = body.replace(
    new RegExp(`]\\(/courses/${course_slug}`, 'g'),
    `](https://.com/courses/${course_slug}`,
  );
...
_This article is part of my open-source [System Design Course](https://github.com//machine-learning) available on Github._
...
(async function main() {
  ...
  for (const [, { name, section, slug }] of articles.entries()) {
    const title = `Machine Learning:  ${name}`;
    const main_image = getBanner(section, course_slug, slug);
    const tags = ['machinelearning', 'systemarchitecture', 'tutorial'];
    const canonical_url = `https://github.com//${course_slug}#${slug}`;
    const series = 'Machine Learning';
